...
import { getEDonusumContent } from '@/lib/i18n/services/integrationSolutions';

const CARD_ICON_NAMES = ['FileCheck', 'FileText', 'Archive', 'BookOpen', 'Receipt', 'ShieldCheck'];


export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    const { lang } = await params;
    const t = getEDonusumContent(lang);
    return generateSeoMetadata({
        lang,
        path: '/services/integration-solutions/e-donusum',
        title: `${t.title} | BTP Solution`,
        description: t.description,
    });
}

export default async function EDonusumPage({ params }: { params: Promise<{ lang: string }> }) {
    const { lang } = await params;
    const t = getEDonusumContent(lang);
    ...
    return (
        <main className="w-full flex flex-col bg-[#060d1a]">
            ...
            <ServiceJsonLd name={t.title} description={t.description} lang={lang} path="/services/integration-solutions/e-donusum" />
            <ServiceHero
