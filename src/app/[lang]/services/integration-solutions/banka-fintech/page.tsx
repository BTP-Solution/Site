...
import { getBankaFintechContent } from '@/lib/i18n/services/integrationSolutions';

const CARD_ICON_NAMES = ['CreditCard', 'ArrowLeftRight', 'Wallet', 'FileCheck2', 'ShieldCheck', 'Landmark'];

export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    const { lang } = await params;
    const t = getBankaFintechContent(lang);
...
export default async function BankaFintechPage({ params }: { params: Promise<{ lang: string }> }) {
    const { lang } = await params;
    const t = getBankaFintechContent(lang);
    ...
    return (
        <main className="w-full flex flex-col bg-[#060d1a]">
            ...
            <ServiceJsonLd name={t.title} description={t.description} lang={lang} path="/services/integration-solutions/banka-fintech" />
            <ServiceHero
